...
const MoreBlogs = () => {
  ...
  return (
    <section className="w-auto h-auto py-8 sm:py-10 md:py-12 lg:py-[48px] gap-6 sm:gap-8 lg:gap-10">
      <div className="mx-3 sm:mx-6 md:mx-10 lg:mx-16 xl:mx-20 gap-4 sm:gap-5 md:gap-6 lg:gap-[28px] space-y-4 sm:space-y-5">
        ...
        {/* Category Filters */}
        <div className="flex items-center justify-center flex-wrap gap-1.5 sm:gap-2 md:gap-3 lg:gap-4 px-2 sm:px-4">
          {categories.map((category) => (
            <button
              key={category}
              onClick={() => setActiveCategory(category)}
              className={`px-2.5 sm:px-3 md:px-4 lg:px-5 xl:px-6 py-1 sm:py-1.5 md:py-2 rounded-full text-[10px] sm:text-xs md:text-sm font-semibold transition-all duration-300 ${
                activeCategory === category
                  ? "border-b-2 text-[#021A54] border-[#021A54]"
                  : "border-b-2 border-transparent hover:text-[#021A54] hover:border-[#021A54]"
              }`}
            >
              {category}
            </button>
